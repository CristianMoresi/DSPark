 ...
 * approximate diagnostics, callable from either of those two threads.
 * Construction and destruction are setup-time (never concurrent with
 * push/pop). The element buffer is plain storage by design: ownership of a
 * slot transfers through the acquire/release index pair (writePos_/readPos_),
 * so under the SPSC contract no slot is ever accessed concurrently --
 * multi-producer or multi-consumer use is out of contract: a data race.
 *
