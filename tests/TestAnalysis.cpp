...
// (replaceable allocation functions, [new.delete]), but strictly pass-through
// unless armed, and they are armed ONLY inside the single-threaded pin below,
// between two calls on the same thread with no other thread alive -- no other
// suite case can ever observe an injected failure.
//
...
// assumed: under ASan+UBSan the whole suite is green with this case injecting
