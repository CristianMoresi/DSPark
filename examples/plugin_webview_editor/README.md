...
One binary is both formats: install it as `.vst3` and as `.clap`. On Linux/X11
the editor embeds WebKitGTK through runtime `dlopen` resolution and
GtkPlug/XEmbed, with the host run loop driving GTK. The generic UI remains the
runtime fallback when WebKitGTK or the required host run-loop capability is
unavailable.
